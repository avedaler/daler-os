...
export function buildDayIcs(iso, s, settings, deals) {
  ...
  ev.push(vevent({
    ...
    desc: "План против факта, три победы, решение завтра.",
  }));
...
export function buildRitualsIcs(iso, settings, days = 30) {
  ...
  return wrap([
    ...
    vevent({ uid: `r-shutdown-${iso}`, iso, start: settings.shutdownTime || "21:30", minutes: 20, title: "DALER OS · Shutdown", desc: "План против факта, три победы, решение завтра.", rrule: rr }),
...
export function buildReminderText(iso, s, settings, deals) {
  ...
  L.push("Тренировка: бицепс / грудь");
  L.push("Не курить · не пить");
  L.push("Законы логики — 20 минут");
