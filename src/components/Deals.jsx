...
export default function Deals({ deals, setDeals, today }) {
  ...
  return <div className="deals-screen">
    <div className="screen-toolbar"><div><span className="kicker">Воронка</span><h2>Следующие действия</h2></div><Btn primary onClick={() => setAdding(true)}><Plus size={16} aria-hidden="true" />Новая сделка</Btn></div>
    <div className="metric-strip deals-metrics"><div><span>Сумма воронки</span><strong>{compactMoney(pipelineValue)}</strong></div><div><span>Требуют действия</span><strong>{counts.today}</strong></div><div><span>Просрочено</span><strong className="red-text">{counts.overdue}</strong></div><div><span>Подписано+</span><strong className="green-text">{counts.signed}</strong></div></div>
    <div className="seg" role="tablist">{[
      ["today", "Сейчас"], ["all", "Все"], ["risk", "Под риском"], ["signed", "Подписано+"],
    ...
    <div className="deal-list">{shown.map((deal) => <DealCard key={deal.id} deal={deal} today={today} onChange={(patch) => update(deal.id, patch)} onDelete={() => setDeals(deals.filter((item) => item.id !== deal.id))} />)}{shown.length === 0 && <EmptyState title="В этом фильтре пусто" text="Добавь сделку кнопкой сверху или назначь следующий шаг в существующей." />}</div>
