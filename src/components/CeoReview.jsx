...
const QUESTIONS = [
  ...
  { key: "gap", title: "Где разрыв между историей и реальностью?", label: "Разрыв", placeholder: "Что говорит версия и что показывают факты" },
  { key: "bottleneck", title: "Одно узкое место", label: "Главное ограничение", placeholder: "Что сильнее всего тормозит результат" },
  { key: "subtraction", title: "Что убрать?", label: "Вычитание", placeholder: "Проект, встреча или обязательство" },
  { key: "nextWeek", title: "Одно главное на следующую неделю", label: "Главный ориентир", placeholder: "Один приоритет и одно доказательство" },
];
...
export default function CeoReview({ date }) {
  ...
  return <div className="ceo-review">
    ...
    {step < QUESTIONS.length ? <Section kicker={`Вопрос ${step + 1} из ${QUESTIONS.length}`} title={question.title}>
      {question.key === "bottleneck" && <ChoiceChips options={["Капитал", "Продажи", "Продукт", "Команда", "Право", "Государство / партнёр", "Личное внимание"]} value={review.bottleneckCategory || ""} onChange={(bottleneckCategory) => update({ bottleneckCategory })} />}
    ...
    </Section> : <Section kicker="Итог" title="Разбор руководителя готов к публикации">
      ...
      <p className="quiet-copy">Главный ориентир следующей недели станет подсказкой в Компасе дня. Ограничение и исключение сохраняются в обзоре как риск и действие.</p>
      <div className="step-actions"><Btn onClick={() => setStep(QUESTIONS.length - 1)}>Назад</Btn><Btn primary onClick={() => update({ done: true, completedAt: new Date().toISOString() })}>{review.done ? "Опубликовано ✓" : "Опубликовать ориентир"}</Btn></div>
    </Section>}
  </div>;
}
